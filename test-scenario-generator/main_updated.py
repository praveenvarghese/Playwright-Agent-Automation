...
from impact_analyzer import analyze_feature_impact, critique_impact_analysis, refine_impact_analysis
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test Scenario Generator and Impact Analyzer')
    ...
    parser.add_argument('--analyze-impact', '-a', action='store_true', help='Perform impact analysis instead of test scenario generation')
    ...
    # Determine whether to perform impact analysis or test scenario generation
    if args.analyze_impact:
        # Impact Analysis workflow
        print("🔹 Analyzing potential impact areas...")
        impact_analysis = analyze_feature_impact(requirement_text, top_k=8)
        
        if impact_analysis:
            print(f"\n✅ Impact Analysis Generated:\n{impact_analysis}")
            
            # Critique the impact analysis
            print("🔹 Critiquing impact analysis...")
            critique_feedback = critique_impact_analysis(impact_analysis)
            
            if critique_feedback:
                print(f"\n✅ Impact Analysis Critique:\n{critique_feedback}")
                
                # Refine the impact analysis based on critique
                print("🔹 Refining impact analysis based on critique...")
                refined_analysis = refine_impact_analysis(requirement_text, impact_analysis, critique_feedback)
                
                # Save the impact analysis output
                output_prefix = os.path.splitext(os.path.basename(input_path))[0]
                save_to_file(f"{output_prefix}_impact_analysis.txt", refined_analysis)
                save_to_file(f"{output_prefix}_impact_critique.txt", critique_feedback)
                
                print(f"\n✅ Final Impact Analysis Saved with prefix: {output_prefix}")
            else:
                print("\n❌ No critique feedback received.")
        else:
            print("\n❌ No impact analysis generated.")
    else:
        # Test Scenario Generation workflow (original functionality)
